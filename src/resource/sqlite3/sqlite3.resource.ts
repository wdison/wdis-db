...
import { META_MODEL_REPO, QRY_CREATE, QRY_DELETE, QRY_DROP, QRY_INSERT, QRY_NATIVE, QRY_SELECT, QRY_UPDATE } from '../resource.constants';
...
import { Sqlite3MetaModelRepo } from './sqlite3.meta.model.repo';
import { Sqlite3Native } from './sqlite3.native';
...
export class Sqlite3Resource extends AbstractSqlResource {
    ...
    constructor() {
        ...
        this.set(QRY_NATIVE, (nativeQry:string)=>new Sqlite3Native(_self, nativeQry));
        ...
        this.set(META_MODEL_REPO, ()=>new Sqlite3MetaModelRepo(_self));
    }
    ...
    public config(conf: ConfResource) {
        ...
        if (conf.options?.connection) {
            repo = conf.options.connection as Database;
        } else {
            repo = new Database(this.conf.url, OPEN_READWRITE | OPEN_CREATE, (err) => {
                if (err) {
